...
| Type       | Usage           | Example                            |
| ---------- | --------------- | ---------------------------------- |
| `feature`  | 기능 개발           | `feature/create-booking`           |
| `fix`      | 버그 수정           | `fix/booking-time-conflict`        |
| `refactor` | 기능 변경 없는 구조 개선  | `refactor/extract-booking-service` |
| `docs`     | 문서 작업           | `docs/update-product-scope`        |
...
| Type       | Usage           | Example                       |
| ---------- | --------------- | ----------------------------- |
| `feat`     | 기능 추가           | `feat: 예약 서비스 등록 기능 추가`       |
| `fix`      | 버그 수정           | `fix: 예약 시간 충돌 검증 수정`         |
| `refactor` | 기능 변경 없는 구조 개선  | `refactor: 예약 가능 시간 계산 로직 분리` |
| `docs`     | 문서 변경           | `docs: 예약 규칙 문서 추가`           |
| `test`     | 테스트 추가 또는 수정    | `test: 예약 취소 테스트 추가`          |
| `chore`    | 설정, 의존성 등 기타 작업 | `chore: 스프링 부트 프로젝트 초기화`      |
...
- 제목은 커밋 메시지와 같은 형식을 사용합니다. 예: `feat: 예약 서비스 등록 기능 추가`
- 본문에는 변경 사항, 테스트 여부, 관련 이슈를 간단히 작성합니다.
- Pull Request 본문은 한글 개조식으로 간결하게 작성합니다.
